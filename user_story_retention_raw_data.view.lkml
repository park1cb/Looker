view: user_story_retention_raw_data {
  derived_table: {
    sql: select distinct cast(u.joined_at + interval '5' hour as date) as joined_at_est,cast(base_dt + interval '5' hour as date) as base_date_est,bookmark.user_id,bookmark.story_id,epi."no",min("no") over (partition by bookmark.user_id,bookmark.story_id order by epi."no") as min_episode
